...
/** A single action that can be chosen. */
 ...
 class Side {
	...
	choose(input) {
		...
		for (const choiceString of choiceStrings) {
            ...
            case 'atk':
                ...
                if (debug){
                    this.battle.add('Changing attack', pokemon);
                    this.battle.add('Old attack', pokemon.storedStats.atk);
                }

                pokemon.storedStats.atk = amt;

                if (debug){
                    this.battle.add('New attack', pokemon.storedStats.atk, '\n');
                }
            ...
            case 'def':
                ...
                if (debug){
                    this.battle.add('Changing defense', pokemon);
                    this.battle.add('Old defense', pokemon.storedStats.def);
                }

                pokemon.storedStats.def = amt;

                if (debug){
                    this.battle.add('New defense', pokemon.storedStats.def, '\n');
                }
            ...
            case 'spe':
                ...
                if (debug){
                    this.battle.add('Changing speed', pokemon);
                    this.battle.add('Old speed', pokemon.storedStats.spe);
                }

                pokemon.storedStats.spe = amt;

                if (debug){
                    this.battle.add('New speed', pokemon.storedStats.spe, '\n');
                }
            ...
            case 'spa':
                ...
                if (debug){
                    this.battle.add('Changing special attack', pokemon);
                    this.battle.add('Old special attack', pokemon.storedStats.spa);
                }

                pokemon.storedStats.spa = amt;

                if (debug){
                    this.battle.add('New special attack', pokemon.storedStats.spa, '\n');
                }
            ...
            case 'spd':
                ...
                if (debug){
                    this.battle.add('Changing special defense', pokemon);
                    this.battle.add('Old special defense', pokemon.storedStats.spd);
                }
                pokemon.storedStats.spd = amt;

                if (debug){
                    this.battle.add('New special defense', pokemon.storedStats.spd, '\n');
                }
            ...
            case 'hp':
                ...
                if (debug){
                    this.battle.add('Changing current health', pokemon);
                    this.battle.add('Old current health', pokemon.hp);
                }

                pokemon.hp = amt;

                if (debug){
                    this.battle.add('New current health', pokemon.hp, '\n');
                }
            ...
            case 'maxhp':
                ...
                if (debug){
                    this.battle.add('Changing max health', pokemon);
                    this.battle.add('Old max health', pokemon.maxhp);
                }

                pokemon.maxhp = amt;

                if (debug){
                    this.battle.add('New max health', pokemon.maxhp, '\n');
                }
            ...
            case 'stats':
                ...
                if (debug) {
                    this.battle.add('Changing stats', pokemon);
                    this.battle.add('hp, atk, def, spa, spd, spe', data);
                }

                pokemon.maxhp = parseInt(data[0]);
                pokemon.hp = pokemon.maxhp;
                pokemon.storedStats.atk = parseInt(data[1]);
                pokemon.storedStats.def = parseInt(data[2]);
                pokemon.storedStats.spa = parseInt(data[3]);
                pokemon.storedStats.spd = parseInt(data[4]);
                pokemon.storedStats.spe = parseInt(data[5]);
            ...
            case 'boosts':
                ...
                if (debug) {
                    this.battle.add('Changing boosts', pokemon);
                    this.battle.add('atk, def, spa, spd, spe, acc, eva', data, '\n');
                }

                pokemon.boosts.atk = parseInt(data[0]);
                pokemon.boosts.def = parseInt(data[1]);
                pokemon.boosts.spa = parseInt(data[2]);
                pokemon.boosts.spd = parseInt(data[3]);
                pokemon.boosts.spe = parseInt(data[4]);
                pokemon.boosts.accuracy = parseInt(data[5]);
                pokemon.boosts.evasion = parseInt(data[6]);
            ...
            case 'moves':
                ...
                new_moves = data.split(" ");

                if (debug){
                    this.battle.add('Changing moves', pokemon);
                    this.battle.add('Old Moves', pokemon.moveSlots.map(({ move }) => move));
                }

                pokemon.moveSlots = [];

                for (let i = 0; i < Math.min(4, new_moves.length); i++){
                    pokemon.moveSlots.push(  this.makeMoveForSlot(new_moves[i])  );
                }

                if (debug){
                    this.battle.add('New Moves', pokemon.moveSlots.map(({ move }) => move), '\n');
                }
            ...
            case 'addmove':
                ...
                if (debug){
                    this.battle.add('Adding moves', pokemon);
                    this.battle.add('Current Moves', pokemon.moveSlots.map(({ move }) => move));
                }

                if (pokemon.moveSlots.length == 4)
                    pokemon.moveSlots[3] = this.makeMoveForSlot(data);
                else
                    pokemon.moveSlots[pokemon.moveSlots.length] = this.makeMoveForSlot(data);
            ...
            case 'types':
                ...
                new_types = data.split(" ");

                if (debug){
                    this.battle.add('Changing types', pokemon);
                    this.battle.add('Old types', pokemon.types);
                }

                pokemon.types = [];

                for (let i = 0; i < Math.min(2, new_types.length); i++){
                    pokemon.types.push(  new_types[i]  );
                }

                if (debug){
                    this.battle.add('New types', pokemon.types, '\n');
                }
            ...
            case 'ability':
                ...
                amt = this.battle.dex.abilities.get(data).id;

                if (debug){
                    this.battle.add('Changing ability', pokemon);
                    this.battle.add('Old ability', pokemon.ability);
                }

                pokemon.ability = amt;

                if (debug){
                    this.battle.add('New ability', pokemon.ability, '\n');
                }
            ...
            case 'item':
                ...
                amt = this.battle.dex.items.get(data).id;

                if (debug){
                    this.battle.add('Changing item', pokemon);
                    this.battle.add('Old item', pokemon.item);
                }

                pokemon.item = amt;

                if (debug){
                    this.battle.add('New item', pokemon.item, '\n');
                }
            ...
            case 'status':
                ...
                // brn , par, slp, psn, tox, frz

                pokemon.setStatus(data);

                if (debug){
                    this.battle.add('Setting status', pokemon);
                    this.battle.add('New status', data);
                }
            ...
            case 'weather':
                ...
                // hail, raindance, sandstorm, 'sunnyday', 'clear'
                if (data === 'clear' || data == ''){
                    this.battle.field.clearWeather();
                } else {
                    this.battle.field.setWeather(data, pokemon);
                }

                if (debug){
                    this.battle.add('Setting weather', data);
                }
            ...
            case 'terrain':
                ...
                // electricterrain, psychicterrain, grassyterrain, 'mistyterrain, 'clear'
                if (data === 'clear' || data == ''){
                    this.battle.field.clearTerrain();
                } else {
                    this.battle.field.setTerrain(data, pokemon);
                }

                if (debug){
                    this.battle.add('Setting terrain', data);
                }
            ...
            case 'pseudoweather':
                ...
                // 'magicroom', 'trickroom', 'wonderroom', 'clear', etc.
                if (data === 'clear' || data == ''){
                    this.battle.field.pseudoWeather = {};
                } else {
                    data = this.battle.dex.conditions.get(data);
                    this.battle.field.pseudoWeather[data.id] = {
                        id: data.id,
                        pokemon,
                        sourceSlot: _optionalChain([pokemon, 'optionalAccess', _12 => _12.getSlot, 'call', _13 => _13()]),
            ...
            case 'sidecondition':
                ...
                // 'lightscreen', etc., 'clear'
                if (data === 'clear' || data == ''){
                    this.sideConditions = {};
                } else {
                    data = this.battle.dex.conditions.get(data);
                    this.sideConditions[data.id] = {
                        id: data.id,
                        target: this,
                        pokemon,
                        sourceSlot: _optionalChain([pokemon, 'optionalAccess', _14 => _14.getSlot, 'call', _15 => _15()]),
                        duration: data.duration,
                    };
                }
            ...
            case 'allmoves':
                ...
                for (let move of this.battle.dex.moves.all()){
    ...
    makeMoveForSlot(moveID)  {
        let move = this.battle.dex.moves.get(moveID);
        if (!move.id){
            throw new Error(`This move is not recognized:${moveID}`);
        }
        ...
        if (debug || movetarget){
            this.battle.add('target', move.name, move.target);
        }
