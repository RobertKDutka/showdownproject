...
def getPikalytics(pokename, num_moves=4, num_items=1, num_abilities=1, num_natures=1):
    ...
    nature_entry = re.compile(
        '<div style="margin-left:10px;display:inline-block;">(?P<nature>.*)</div>'
    )

    ev_entry = re.compile(
        '<div style="display:inline-block;">(?P<ev>[0-9]{1,4})/?</div>'
    )
...
def calcStats(pokemon, pika_stats, lvl=50):
    ...
    for stat in stat_names:
        pokemon.stats[stat] = (
            2 * base_stats[stat] + pokemon.ivs[stat] + evs[0][stat] // 4
        ) * lvl // 100 + 5
        pokemon.evs[stat] = evs[0][stat]

    pokemon.stats["hp"] = (
        (2 * base_stats["hp"] + pokemon.ivs[stat] + evs[0]["hp"] // 4) * lvl // 100
        + lvl
        + 10
    )
