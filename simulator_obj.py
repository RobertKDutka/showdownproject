...
class simulator:
    ...
    def __init__(self, my_team, oppo_team):
        ...
        print(oppo_team)  # REMOVE ME IF CLEANING
    ...
    def findBestMove(self, is_random=False, top_moves=5):
        ...
        move_targets = re.search(
            r"p1\ move\ (?P<move1>[1-4])(\ )?(?P<target1>(-1|-2|1|2))?\ *(,\ move\ (?P<move2>[1-4])(\ )?(?P<target2>(-1|-2|1|2))?)?",
            best_move,
        )
    ...
    def randomMove(self, p1moves):
        ...
        move_targets = re.search(
            "p1\ move\ (?P<move1>[1-4])(\ )*(?P<target1>(-1|-2|1|2))?(, move\ (?P<move2>[1-4])(\ )*(?P<target2>(-1|-2|1|2))?)?",
            random_move,
        )
    ...
    def parse_my_team(self, my_team):
        # Todo add comments to ll of this unintelligible regex
        poke_item = re.compile("(?P<pokemon>.+?)( \([MF]\))?\ @\ (?P<item>.+)")
        ability = re.compile("Ability:\ (?P<ability>.+)")
        level = re.compile("level:\ (?P<ability>.+)")
        evs = re.compile(
            "EVs:\ ((?P<hp>[0-9]+)\ HP)?((\ \/\ )?(?P<atk>[0-9]+)\ Atk)?((\ \/\ )?(?P<def>[0-9]+)\ Def)?((\ \/\ )?(?P<spa>[0-9]+)\ SpA)?((\ \/\ )?(?P<spd>[0-9]+)\ SpD)?((\ \/\ )?(?P<spe>[0-9]+)\ Spe)?"
        )
        nature = re.compile("(?P<Nature>.+)\ Nature")
        ivs = re.compile(
            "IVs:\ ((?P<hp>[0-9]+)\ HP)?((\ \/\ )?(?P<atk>[0-9]+)\ Atk)?((\ \/\ )?(?P<def>[0-9]+)\ Def)?((\ \/\ )?(?P<spa>[0-9]+)\ SpA)?((\ \/\ )?(?P<spd>[0-9]+)\ SpD)?((\ \/\ )?(?P<spe>[0-9]+)\ Spe)?"
        )
        ...
        for i in range(6):
            ...
            print("Line 192: ", pokename)  # REMOVE ME IF CLEANING

            # poke_stats = stats, moves, items, abilities, nature, typing
            # stats is a dict, the rest are a list
            ...
            line = lines.pop(0)
            print("Line 218: ", line)  # REMOVE ME IF CLEANING
            match = ivs.match(line)
            for stat in stat_names:
                if match is not None and match.group(stat) != None:
    ...
    def parse_battle_history(self, file="battle-history.txt"):
        ...
        for line in f:
            ...
            match = re.search(
                "The\ opposing\ (?P<poke>.+)\ lost\ (?P<lost_hp>[0-9]+)%\ of\ its\ health!",
                line,
            )
            if match:
                for poke in self.ms.team2.full:
            ...
            # Our pokemon lost hp
            match = re.search(
                "(?P<poke>.+)\ lost\ (?P<lost_hp>[0-9\.]+)%\ of\ its\ health!", line
            )
            if match:
            ...
            if match and opposing:
                item = re.search(
                    "opposing\ (?P<poke>.+)\ and\ found\ its\ (?P<item>.+)(!)?", line
                )
                for poke in self.ms.team2.full:
            ...
            match = re.search(
                "restored\ a\ little\ HP\ using\ its\ (?P<item>.+)(!)?", line
            )
            if match and opposing:
                match2 = re.search("opposing\ (?P<poke>.+)\ ", line)
                for poke in self.ms.team2.full:
            ...
            match = re.search(
                "is\ about\ to\ be\ attacked\ by\ its\ (?P<item>.+)(!)?", line
            )
            if match and opposing:
                match2 = re.search("opposing\ (?P<poke>.+)\ ", line)
                for poke in self.ms.team2.full:
            ...
            if match and opposing:
                match2 = re.search(
                    "\(The\ opposing\ (?P<poke>.+)\ ate\ its\ (?P<item>.+)!\)", line
                )
                for poke in self.ms.team2.full:
                    if poke.name == match2.group("poke"):
                        poke.item = match2.group("item")
